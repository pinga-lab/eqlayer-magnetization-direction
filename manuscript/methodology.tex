...
\label{eq:q_vector}
\end{equation}
For convenience, we call $\mathbf{q}$ as direction vector.
...
\label{eq:tfa_pred_i}
\end{equation}
where $\mathbf{p}$ is an $M \times 1$ vector (the magnetic-moment vector) 
whose $j$th element, $j = 1, \dots, M$, is the 
magnetic moment intensity $p_{j}$ of the $j$th dipole, $\mathbf{s}$ is an $(M + 2) \times 1$ 
vector (the parameter vector) given by 
\begin{equation}
...
\end{equation}
and $\mathbf{g}_{i} (\mathbf{q})$ is another $M \times 1$ vector whose $j$th element is defined 
...
$(x'', y'', z_{c}) = (x_{j}, y_{j}, z_{c})$.
These equations show that the predicted total-field anomaly 
$\Delta T_{i}(\mathbf{s})$ (equation \ref{eq:tfa_pred_i}) has a linear relation with 
the magnetic-moment vector $\mathbf{p}$ and a nonlinear relation with the direction vector 
$\mathbf{q}$ (equation \ref{eq:q_vector}).


\subsection{Inverse problem}

%%%% Defining the objective function
Let $\mathbf{\Delta T}^{o}$ and $\mathbf{\Delta T} (\mathbf{s})$ be $N \times 1$ vectors whose 
$i$th elements $\Delta T_{i}^{o}$ and $\Delta T_{i}(\mathbf{s})$ (equation \ref{eq:tfa_pred_i}) 
are the total-field anomalies produced, respectively, by the magnetic sources and the
discrete equivalent layer at the point $(x_{i},y_{i},z_{i})$, $i = 1, \dots, N$. 
For convenience, $\mathbf{\Delta T}^{o}$ and $\mathbf{\Delta T} (\mathbf{s})$ are called
observed and predicted data vector, respectively. In order to estimate the uniform 
magnetization direction of the sources, we solve the following inverse problem:
\begin{subequations}
	\begin{align}
	& \text{minimizing}
...
\mathbf{J}^{k^T} \mathbf{\Delta s}^k + 
\frac{1}{2} \mathbf{\Delta s}^{k^T} \mathbf{H}^{k} \mathbf{\Delta s}^k  \: ,
\label{eq:sec_ord_goal}
\end{equation}
where $\mathbf{\Delta s}^k$ is a perturbation on the parameter vector $\mathbf{s}^k$ and the terms 
$\mathbf{J}^{k}$ and $\mathbf{H}^{k}$ are, respectively, the gradient vector and the 
Hessian matrix evaluated at $\mathbf{s}^{k}$.
To estimate the perturbation vector $\bar{\mathbf{\Delta s}}^k$ minimizing the expanded function 
(equation \ref{eq:sec_ord_goal}), we take the gradient with respect to $\mathbf{\Delta s}^k$,
set the result equal to the null vector and obtain the linear system
\begin{equation}
...
We approximate this linear system by
\begin{equation}
\left[
\begin{array}{c|c}
...
\end{equation}
in which $\mathbf{0}$ is a matrix containing all elements equal to zero, 
...
is a correction on the direction vector $\mathbf{q}$ and the terms 
...
Notice that we use an approximated Hessian matrix having null cross-derivatives. 

The gradient vector $\mathbf{J}_{q}^{k}$ and the Hessian matrix $\mathbf{H}_{pp}^{k}$ 
(equation \ref{eq:linear_sys_GN_block}) are, respectively,
\begin{equation}
\mathbf{J}_{p}^{k} = -2 \mathbf{G}_{p}^{k^T} [ \mathbf{\Delta T}^{o} - 
\mathbf{\Delta T} (\mathbf{s}^k) ] + 2\mu f_{0}^{k} \mathbf{p}^{k} 
\label{eq:grad_p}
\end{equation}   
and   
\begin{equation}
...
function $g_{ij}(\mathbf{s}^{k})$ (equation \ref{eq:g_ij}), 
$\mathbf{I}$ is the $M \times M$ identity matrix and $f_{0}^{k}$ is a  normalizing factor 
equal to
\begin{equation}
f_{0}^{k} = \dfrac{trace \left(\mathbf{G}_{p}^{k^T} \mathbf{G}_{p}^{k} \right)}{M} \, ,
\label{eq:norm_factor}
\end{equation}
and $M$ is the total number of dipoles composing the equivalent layer. 

PAREI AQUI

From the correction $\bar{\mathbf{\Delta p}}^{k} = \bar{\mathbf{p}}^{k+1} - \bar{\mathbf{p}}^{k}$, we can conclude that the linear system to be solved is given by
...
The magnetic-moment $\bar{\mathbf{p}}$ and direction $\bar{\mathbf{q}}$ vectors are obtained through an inverse problem of minimizing the difference between the observed data $ \mathbf{\Delta T}^o $ and the predicted data $\mathbf{\Delta T} (\mathbf{s})$. As we can notice, the solution of the inversion is given by solving two linear systems shown in equation \ref{eq:linear_sys_p} and equation \ref{eq:linear_sys_q}. For this reason, we propose a nested algorithm for solving the inverse problem in two steps.           

...
\label{eq:tfa_pred_i}
\end{equation}
where $\mathbf{p}$ is an $M \times 1$ vector whose $j$th element, $j = 1, \dots, M$, is the 
...
\mathbf{H}_{ij} \, \hat{\mathbf{m}}(\mathbf{q}) \: .
\label{eq:g_ij}
\end{equation}
In this equation, $\mathbf{H}_{ij}$ is a $3 \times 3$ matrix given by
\begin{equation}
...
the magnetic moment $\mathbf{p}$ and a nonlinear relation with the vector 

...
layer, where $z < z_{c}$. 
By equaling each term to zero and rewriting the second derivatives of the surface 
integral $\Phi(x, y, z)$ (equation \ref{eq:Phi-surface-integral}):
\begin{equation}
...
\end{equation}
$\mathbf{q}$ is the direction vector (equation \ref{eq:q_vector}), $\mathbf{p}$ is an 
...
\label{eq:Mij-matrix}
\end{equation}
where $\partial_{\alpha\beta} \frac{1}{r}$ represent the second derivatives,
...
and a nonlinear relation with the direction vector $\mathbf{q}$ (equation \ref{eq:q_vector}).
...
$\mathbf{p}$ and $\mathbf{q}$, respectively.%, both evaluated at the estimated 
%magnetic-moment vector $\bar{\mathbf{p}}^{k}$. 
...
function $g_{ij}(\bar{\mathbf{q}}^{k})$ (equation \ref{eq:g_ij}) evaluated at the estimated
direction vector $\bar{\mathbf{q}}^{k}$, $\mathbf{I}$ is the $M \times M$ identity matrix and 
...
inclination $I$ and the declination $D$ of the sources total magnetization.

...
and $\Gamma(x, y, z)$ (equation \ref{eq:Gamma-volume-integral}) resembles, respectively, 
...
$I$ and $D$ (equation \ref{eq:mag_vec}).
Because the coefficients $a_{\alpha\beta}$ are always valid for any value, we conclude that the five 
linearly independent harmonic functions in brackets must be identically zero for all points 
$(x, y, z)$ above the equivalent layer, where $z < z_{c}$. By assuming that, in the case which the equivalent layer has the same magnetization direction of the magnetic sources, the scalar functions $\Gamma(x, y, z)$ and $\Phi(x, y, z)$ are equal. Thus, a possible solution for equation \ref{eq:tfanomaly-alternative-equality} can be given by 
...
\end{equation}
where $(x'', y'', z_{c})$ is a point on the equivalent layer. Consequently, the second derivative of the equation \ref{eq:Gamma_integral_equation} is equal to 
...
\end{equation}
It can be shown (see Appendix A) that the integral equation \ref{eq:Gamma_integral_equation} has a solution
\begin{equation}
...
\end{equation}
This factor is used with the purpose to make a trade-off between the two terms on the right side of equation \ref{eq:positivity_goal_function}a along the iterative process. The gradient vector $\mathbf{J}_{q}^{k}$ and the Hessian matrix $\mathbf{H}_{qq}^{k}$ 
...
The method of the choice of layer depth is based on a classical approach proposed by \cite{dampney1969}. This author pointed out that the layer depth should satisfy an interval from $2.5$ to $6$ times the grid spacing. It should be noticed that the rule proposed by \cite{dampney1969} was applied on an evenly spaced data. However, in this work, the interval was found empirically and the range can vary from $2$ to $3$ times the greater grid spacing using simulated flight lines of an airborne survey.
